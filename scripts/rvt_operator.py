...
try:
    ...
    sys.path.insert(0,
                    os.path.join(
                        os.path.dirname(__file__), '../pyrvt'))
    import pyrvt
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        If the BT12 method is used, then the magnitude, distance and region must
        be provided.'''
    )
    ...
    if args.operation == 'sa2fa':
        pyrvt.tools.operation_sa2fa(args.src, args.dst, args.damping, args.method, args.fixed_spacing)
    elif args.operation == 'fa2sa':
        pyrvt.tools.operation_fa2sa(args.src, args.dst, args.damping, args.method, args.fixed_spacing)
    else:
